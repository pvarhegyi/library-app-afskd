...
def test_borrow_book_success():
    ...
    mock_book = Book(id=1, title="Alice in Wonderland", author="Lewis Caroll", available=True)
    mock_borrower = Borrower(id=1, name="Carol", email="carol@example.com")
    mock_db.query.return_value.filter.return_value.first.side_effect = [mock_book, mock_borrower]
...
def test_borrow_book_book_not_found():
    ...
    mock_db.query.return_value.filter.return_value.first.side_effect = [None, mock_borrower]
    service = BookService(mock_db)
...
def test_borrow_book_borrower_not_found():
    mock_db = MagicMock(spec=Session)
    mock_book = Book(id=1, title="Winnie-the-Pooh", author="A. A. Milne", available=True)
...
def test_borrow_book_already_borrowed():
    mock_db = MagicMock(spec=Session)
    mock_book = Book(id=1, title="Pride and Prejudice", author="Jane Austen", available=False)
